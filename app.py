...
min_date = df["Order Date"].min() if not df.empty else df_original["Order Date"].min()
max_date = df["Order Date"].max() if not df.empty else df_original["Order Date"].max()
...
total_expenses = total_sales - total_profit
...
if df.empty:
    ...
    # ---- Pie Chart and Revenue vs. Expenses Bar Chart ----
    st.subheader("Sales Distribution & Financial Overview")
